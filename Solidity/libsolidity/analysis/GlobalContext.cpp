...
m_magicVariables(vector<shared_ptr<MagicVariableDeclaration const>>{
	...
	////// hdsnark
	// proof, data_hash, result (such as premium), coefficent(12)
	...
	////////gyh:　Groupsign(1个输入)　输出为ｂｏｏｌ类型  (19)
	make_shared<MagicVariableDeclaration>("veriGroupsign", make_shared<FunctionType>(strings{"string memory"}, strings{"uint256"}, FunctionType::Kind::veriGroupsign, false, StateMutability::Pure)),
	make_shared<MagicVariableDeclaration>("verhfProof", make_shared<FunctionType>(strings{"string memory"}, strings{"uint256"}, FunctionType::Kind::verhfProof, false, StateMutability::Pure)),
	////////
